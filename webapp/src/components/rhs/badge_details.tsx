...
class BadgeDetailsComponent extends React.PureComponent<Props, State> {
  ...
  componentDidUpdate(prevProps: Props, prevState: State) {
    if (
      this.state.badge !== prevState.badge &&
      this.state.badge &&
      !systemEmojis.has(this.state.badge.name)
    ) {
  ...
  render() {
    ...
    if (this.props.badgeID == null) {
      return <div>{"Badge nicht gefunden."}</div>;
    }
      ...
      <div className="BadgeDetails">
        ...
        <div className="badge-info">
          ...
          <div className="badge-text">
            ...
            <div className="badge-type">{"Typ: " + badge.type_name}</div>
            <div className="created-by">{`Erstellt: ${badge.created_by_username}`}</div>
          </div>
        </div>
        <div>
          <b>{"Vergeben an:"}</b>
