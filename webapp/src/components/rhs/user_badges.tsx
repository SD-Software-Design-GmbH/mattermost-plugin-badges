...
class UserBadges extends React.PureComponent<Props, State> {
  ...
  render() {
    if (!this.props.user) {
      return <div>{"User nicht gefunden."}</div>;
    }
    ...
    if (!this.state.badges || this.state.badges.length === 0) {
      return <div>{"Noch keine Auszeichnungen."}</div>;
    }
    ...
    let title = "Meine Auszeichnungen";
    if (!this.props.isCurrentUser) {
      title = `@${this.props.user.username}'s Auszeichnungen`;
    }
    return (
