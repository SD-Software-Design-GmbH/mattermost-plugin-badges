...
class BadgeList extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    for (let i = 0; i < toShow; i++) {
      ...
      let reason = null;
      if (badge.reason) {
        reason = <div>{"Warum? " + badge.reason}</div>;
      }
        ...
        <OverlayTrigger
          overlay={
            <Tooltip id="badgeTooltip">
              ...
              {reason}
              <div>{`Erhalten: ${time.toDateString()}`}</div>
            </Tooltip>
          }
        >
          <span>
            <a onClick={() => this.onBadgeClick(badge)}>
              <BadgeImage badge={badge} size={BADGE_SIZE} />
            </a>
          </span>
        </OverlayTrigger>
      );
    ...
    if (nBadges > MAX_BADGES) {
      andMore = (
        <OverlayTrigger
          overlay={
            <Tooltip id="badgeMoreTooltip">
              {`und ${nBadges - MAX_BADGES} mehr. Klicke um alle zu sehen.`}
            </Tooltip>
          }
        >
          <button id="showMoreButton" onClick={this.onMoreClick}>
    ...
    return (
      <div id="badgePlugin">
        ...
        {loading}
        <button id="grantBadgeButton" onClick={this.onGrantClick}>
